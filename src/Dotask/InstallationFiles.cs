...
internal static class InstallationFiles
{
  ...
  {
    NotLink(path);
    try { return JsonSerializer.Deserialize<T>(File.ReadAllBytes(path), Json) ?? throw new JsonException("Empty record."); } catch (JsonException ex) { throw new TaskException($"Invalid installation record '{path}': {ex.Message}"); }
  }
  ...
  internal static FileStream Lock( string directory )
  {
    ...
    try { return new FileStream(path, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None, 1, FileOptions.DeleteOnClose); } catch (IOException) { throw new TaskException($"Another installer may be using '{directory}'. Retry after it finishes."); }
  }
